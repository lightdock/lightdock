...
class TestRegressionDFIREShort(RegressionTest):
    ...
    def setup(self):
        ...
        self.golden_data_path = os.path.normpath(os.path.dirname(os.path.realpath(__file__))) + \
                                '/golden_data/regression_dfire_short/'
    ...
    def test_lightdock_2uuy_15_steps_50_glowworms_1_cluster(self):
        ...
        command = "lightdock %s %s %d %d %d -c 1 -f %s -s " \
                  "dfire > test_lightdock.out" % (self.test_path + '2UUY_rec.pdb',
...
class TestRegressionDFIRELong(RegressionTest):
    ...
    def setup(self):
        ...
        self.ini_test_path()
        self.golden_data_path = os.path.normpath(os.path.dirname(os.path.realpath(__file__))) + \
                                '/golden_data/regression_dfire_long/'
    ...
    def test_lightdock_1ppe_10_steps_100_glowworms(self):
        if os.environ.has_key('LIGHTDOCK_LONG_TEST') and os.environ['LIGHTDOCK_LONG_TEST'] == 'true':
            ...
            command = "lightdock %s %s %d %d %d " \
                      "-f %s -s dfire > test_lightdock.out" % (self.test_path + '1PPE_rec.pdb',
