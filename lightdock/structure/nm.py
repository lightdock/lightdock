...
def calculate_nmodes(pdb_file_name, n_modes, molecule):
    ...
    if not backbone_atoms:
        raise NormalModesCalculationError("Error selecting backbone atoms (protein or nucleic)")
    ...
    if num_atoms_prody != (molecule.protein_num_atoms + molecule.nucleic_num_atoms):
        log.error("Number of atoms in ProDy (%d) vs LightDock (%d)" % (num_atoms_prody,
            molecule.protein_num_atoms + molecule.nucleic_num_atoms))
    ...
    for lightdock_atom, prody_atom in zip (molecule.get_atoms(protein=True, nucleic=True, dummy=False),
