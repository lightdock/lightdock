...
class TestCalculateReferencePoints(RegressionTest):
    ...
    def test_calculate_4IZ7_B(self):
        ...
        shutil.copyfile(self.golden_data_path / '4IZ7_B_noh.pdb', self.test_path / '4IZ7_B_noh.pdb')

        command = f"lgd_calculate_reference_points.py {self.test_path / '4IZ7_B_noh.pdb'} > test.out"
        os.system(command)

        assert filecmp.cmp(self.golden_data_path / '4IZ7_B_noh.pdb.vol',
                           self.test_path / '4IZ7_B_noh.pdb.vol')
        assert filecmp.cmp(self.golden_data_path / '4IZ7_B_noh.pdb.vol.pdb',
